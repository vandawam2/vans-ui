...
function KeyLib:Init(Config)
    ...
    local function FetchScript(inputKey)
        ...
        if success and response then
            ...
            if response.StatusCode == 200 then
                -- Cek safety: Kadang server kirim error tapi statusnya 200
                if response.Body:sub(1,1) == "{" then
                    local isJson, data = pcall(function() return HttpService:JSONDecode(response.Body) end)
                    if isJson and data and data.valid == false then
                        return false, data.message or "Unknown Error"
                    end
                end
                -- Kembalikan Script Raw
                return true, response.Body 
            
            -- [JIKA ERROR (400, 403, 404, etc)]
            elseif response.StatusCode >= 400 then
                -- Coba decode pesan error dari server
                local isJson, data = pcall(function() return HttpService:JSONDecode(response.Body) end)
                
                if isJson and data and data.message then
                    return false, data.message -- CONTOH: "Key Expired!"
                else
                    return false, "Error " .. response.StatusCode .. " (Invalid Key)"
                end
            end
        else
    ...
    if SavedKey then
        ...
        local LoadStatus = Instance.new("TextLabel") LoadStatus.Size = UDim2.new(1, 0, 0, 20) LoadStatus.Position = UDim2.new(0, 0, 0, 40) LoadStatus.BackgroundTransparency = 1 LoadStatus.Text = "Checking saved key..." LoadStatus.TextColor3 = Color3.fromRGB(180, 180, 180) LoadStatus.Font = Enum.Font.Gotham LoadStatus.TextSize = 12 LoadStatus.Parent = LoadFrame
    ...
    local MainFrame = Instance.new("Frame") MainFrame.Name = "MainFrame" MainFrame.Size = UDim2.new(0, 380, 0, 220) MainFrame.Position = UDim2.new(0.5, -190, 0.5, -110) MainFrame.BackgroundColor3 = Color3.fromRGB(20, 20, 20) MainFrame.BorderSizePixel = 0 MainFrame.ClipsDescendants = true MainFrame.BackgroundTransparency = 1 MainFrame.Parent = ScreenGui
    ...
    local InputFrame = Instance.new("Frame") InputFrame.Size = UDim2.new(1, -30, 0, 40) InputFrame.Position = UDim2.new(0, 15, 0, 60) InputFrame.BackgroundColor3 = Color3.fromRGB(30, 30, 30) InputFrame.BorderSizePixel = 0 InputFrame.Parent = MainFrame
    ...
    local InputBox = Instance.new("TextBox") InputBox.Size = UDim2.new(1, -20, 1, 0) InputBox.Position = UDim2.new(0, 10, 0, 0) InputBox.BackgroundTransparency = 1 InputBox.Text = "" InputBox.PlaceholderText = "Paste Key Here..." InputBox.PlaceholderColor3 = Color3.fromRGB(100, 100, 100) InputBox.TextColor3 = Color3.fromRGB(255, 255, 255) InputBox.Font = Enum.Font.Gotham InputBox.TextSize = 14 InputBox.Parent = InputFrame
    ...
    InputBox:GetPropertyChangedSignal("Text"):Connect(function()
        if InputStroke.Color == Color3.fromRGB(255, 80, 80) then TweenService:Create(InputStroke, TweenInfo.new(0.2), {Color = Color3.fromRGB(50, 50, 50)}):Play() TweenService:Create(InputBox, TweenInfo.new(0.2), {TextColor3 = Color3.fromRGB(255, 255, 255)}):Play() StatusLabel.Text = "" end
    end)
    ...
    CreateButton("Verify", Color3.fromRGB(200, 150, 40), function()
        ...
        if success then
            ...
            writefile(KeyFileName, InputKey) -- Simpan Key
            ...
            -- ERROR (Tampilkan Pesan Spesifik dari Server)
