...
function KeyLib:Init(Config)
    ...
    -------------------------------------------------------------------------
    -- [LOGIC UTAMA] FETCH SCRIPT DENGAN ERROR HANDLING KUAT
    -------------------------------------------------------------------------
    local function FetchScript(inputKey)
        ...
        if success and response then
            ...
            if response.StatusCode == 200 then
                -- Cek apakah server malah mengirim JSON error di status 200 (kadang terjadi)
                ...
                -- Jika aman, kembalikan Raw Script
                return true, response.Body 
            
            -- [CASE 2] ERROR CLIENT (400, 401, 403, 404)
            elseif response.StatusCode >= 400 and response.StatusCode < 500 then
                -- Coba baca pesan error dari JSON server
                local isJson, data = pcall(function() return HttpService:JSONDecode(response.Body) end)
                
                if isJson and data and data.message then
                    -- Server mengirim: { "message": "Key Kadaluarsa!" }
                    return false, data.message 
                else
                    -- Server mengirim plain text error
                    return false, response.Body or "Error " .. response.StatusCode
                end
    ...
    -------------------------------------------------------------------------
    local SavedKey = nil
    if isfile(KeyFileName) then SavedKey = readfile(KeyFileName) end
    ...
    if SavedKey then
        ...
        if success then
            ...
            -- Tampilkan Error Spesifik (misal: "Key Expired")
    ...
    local MainFrame = Instance.new("Frame") MainFrame.Size = UDim2.new(0, 380, 0, 220) MainFrame.Position = UDim2.new(0.5, -190, 0.5, -110) MainFrame.BackgroundColor3 = Color3.fromRGB(20, 20, 20) MainFrame.BackgroundTransparency = 1 MainFrame.BorderSizePixel = 0 MainFrame.Parent = ScreenGui
    ...
    InputBox:GetPropertyChangedSignal("Text"):Connect(function() if InputStroke.Color == Color3.fromRGB(255, 80, 80) then TweenService:Create(InputStroke, TweenInfo.new(0.2), {Color = Color3.fromRGB(50, 50, 50)}):Play() StatusLabel.Text = "" end end)
    ...
    CreateButton("Verify", Color3.fromRGB(200, 150, 40), function()
        ...
        StatusLabel.Text = "Verifying..." StatusLabel.TextColor3 = Color3.fromRGB(255, 255, 100)
        ...
        if success then
            ...
            TweenService:Create(MainFrame, TweenInfo.new(0.5), {BackgroundTransparency = 1, Size = UDim2.new(0, 380, 0, 0)}):Play() TweenService:Create(Blur, TweenInfo.new(0.5), {Size = 0}):Play() task.wait(0.5) ScreenGui:Destroy() Blur:Destroy()
            
            local loadFunc, err = loadstring(result)
            if loadFunc then loadFunc() if Settings.Callback then Settings.Callback() end else warn("Script Error: "..tostring(err)) end
        else
            -- [PERBAIKAN] Tampilkan pesan error spesifik dari Server
